"""In-memory vector store backend for knowledge bases."""
...
class InMemoryKnowledgeBackend:
    ...
    async def store(self, chunks: list[Chunk], embeddings: list[list[float]]) -> None:
        # Replace existing data to keep storage idempotent across rebuilds
    ...
    async def search(self, query_embedding: list[float], top_k: int = 5) -> list[ScoredChunk]:
        async with self._lock:
            ...
            return [
                ScoredChunk(chunk=self._chunks[i], score=s, retrieval_method="dense")
                for i, s in ranked
            ]
